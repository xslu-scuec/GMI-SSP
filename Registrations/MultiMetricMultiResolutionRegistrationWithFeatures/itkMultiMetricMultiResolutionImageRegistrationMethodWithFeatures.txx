...
#ifndef _itkMultiMetricMultiResolutionImageRegistrationMethodWithFeatures_txx
#define _itkMultiMetricMultiResolutionImageRegistrationMethodWithFeatures_txx
...
#define itkImplementationSetMacro(_name,_type) \
  ...
  void \
  MultiMetricMultiResolutionImageRegistrationMethodWithFeatures<TFixedImage,TMovingImage> \
...
namespace itk
{
  ...
  {
    this->SetMetric( CombinationMultiInputMetricType::New() );
  } // end Constructor()
  ...
  */
  ...
  {
    CombinationMultiInputMetricType * testPtr = dynamic_cast<CombinationMultiInputMetricType *>( _arg );
    if ( testPtr )
    {
      if ( this->m_CombinationMultiInputMetric != testPtr )
      {
        this->m_CombinationMultiInputMetric = testPtr;
        this->Superclass::SetMetric( this->m_CombinationMultiInputMetric );
        this->Modified();
      }
    }
    else
    {
      itkExceptionMacro( << "The metric must be of type CombinationMultiInputImageToImageMetric!" );
    }
  ...
  */
  ...
  ::Initialize() throw (ExceptionObject)
  {
    ...
    this->GetCombinationMultiInputMetric()->SetTransform( this->GetTransform() );

    /** Setup the metric: the images. */
    this->GetCombinationMultiInputMetric()->SetNumberOfFixedImages( this->GetNumberOfFixedImages() );
    this->GetCombinationMultiInputMetric()->SetNumberOfMovingImages( this->GetNumberOfMovingImages() );

    for ( unsigned int i = 0; i < this->GetNumberOfFixedImagePyramids(); ++i )
    {
      this->GetCombinationMultiInputMetric()->SetFixedImage(
        this->GetFixedImagePyramid( i )->GetOutput( this->GetCurrentLevel() ), i );
    }

    for ( unsigned int i = 0; i < this->GetNumberOfMovingImagePyramids(); ++i )
    {
      this->GetCombinationMultiInputMetric()->SetMovingImage(
    ...
    {
      this->GetCombinationMultiInputMetric()->SetFixedImageRegion(
    ...
    {
      this->GetCombinationMultiInputMetric()->SetInterpolator( this->GetInterpolator( i ), i );
    }

    for ( unsigned int i = 0; i < this->GetNumberOfFixedImageInterpolators(); ++i )
    {
      this->GetCombinationMultiInputMetric()
        ->SetFixedImageInterpolator( this->GetFixedImageInterpolator( i ), i );
    }

    /** Initialize the metric. */
    this->GetCombinationMultiInputMetric()->Initialize();
...
#endif // end #ifndef _itkMultiMetricMultiResolutionImageRegistrationMethodWithFeatures_txx
